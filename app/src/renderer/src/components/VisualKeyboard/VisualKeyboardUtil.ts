...
import { keyShortLabels } from './VisualKeyboardLayout'
import { Bind, BindType, Macro_Bind, TimedMacro_Bind } from '../../../../models/Bind'
...
const bindTypeColors: Record<string, string> = {
...
export const getKeyBindingColors = (key: string, layer: Layer): string[] => {
  const binds = Array.from(layer.remappings.entries())
    ...
    .map(([, bind]) => bind as Bind)

  // If any macro/timed macro, use their color for the whole key
  if (binds.some((b) => b instanceof Macro_Bind)) return [bindTypeColors[BindType.Macro]]
  if (binds.some((b) => b instanceof TimedMacro_Bind)) return [bindTypeColors[BindType.TimedMacro]]

  // If multiple types, show all distinct types (no duplicates)
  const uniqueTypes = Array.from(new Set(binds.map((b) => b.bind_type)))
  const colors = uniqueTypes.map((type) => bindTypeColors[type] || '#d1d5db')
